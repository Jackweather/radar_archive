...
def prepare_motion_field(reflectivity: np.ma.MaskedArray) -> np.ndarray | None:
    prepared = np.asarray(np.ma.filled(reflectivity, 0.0), dtype=np.float32)
    prepared = np.where(prepared >= STORM_MOTION_SAMPLE_DBZ, prepared, 0.0)
    if int(np.count_nonzero(prepared)) < STORM_MOTION_MIN_SIGNAL_PIXELS:
        return None
    ...
    prepared /= max_value
    return gaussian_filter(prepared, sigma=1.2)
...
def estimate_storm_motion(
    ...
    current_field = prepare_motion_field(current_reflectivity)
    previous_field = prepare_motion_field(previous_reflectivity)
    ...
    stride = 2 if max(current_field.shape) >= 700 else 1
    current_field = current_field[::stride, ::stride]
    previous_field = previous_field[::stride, ::stride]
    ...
    for row_shift in range(-max_shift, max_shift + 1):
        ...
        for column_shift in range(-max_shift, max_shift + 1):
            ...
            active_mask = (current_window > 0.0) | (previous_window > 0.0)
            if int(np.count_nonzero(active_mask)) < minimum_signal_pixels:
                continue

            current_values = current_window[active_mask]
            previous_values = previous_window[active_mask]
            denominator = float(np.linalg.norm(current_values) * np.linalg.norm(previous_values))
            if denominator <= 0.0:
                continue

            score = float(np.dot(current_values, previous_values) / denominator)
            if score > best_score:
                best_score = score
